...
const AddContent:React.FC<AddContentProps> = ({isOpen,changeOpen}) => {
  ...
  const [data, setData] = useState<{ title: string; link: string; tags: string[] }>({title:"",link:"",tags:[]})
  ...
  return (
    <div className={`w-screen h-screen bg-ppurple-600 absolute top-0 left-0 bg-opacity-50 flex justify-center items-center ${isOpen ? "block" : "hidden"}`} onClick={onClose}>
      <form onSubmit={handleSubmit} className="bg-white w-96 h-fit h-max-[500px] opacity-100 rounded-lg flex flex-col items-center" onClick={(e)=>{e.stopPropagation()}}>
        ...
        <TagsInput tags={data.tags} setTags={(newTags) => setData({ ...data, tags: newTags })}/>
        <Button text="Submit" type="primary" newClasses="w-72 mt-5 justify-center mb-5" disabled={!(data.title&&data.link)}/>
      </form>
    </div>
  )
}
